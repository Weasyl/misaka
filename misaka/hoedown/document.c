...
char_emphasis(hoedown_buffer *ob, hoedown_document *doc, uint8_t *data, size_t offset, size_t size)
{
	...
	if (doc->ext_flags & HOEDOWN_EXT_NO_INTRA_EMPHASIS) {
		if (offset > 0 && (data[-1] == '_' || isalnum(data[-1])))
			return 0;
	}
...
static size_t
parse_ordered_list_initial_prefix(hoedown_buffer *ob, uint8_t *data, size_t size)
{
...
parse_list(hoedown_buffer *ob, hoedown_document *doc, uint8_t *data, size_t size, hoedown_list_flags flags)
{
	...
	uint8_t *ol_prefix = NULL;
	uint8_t *prefix_start = data;

	work = newbuf(doc, BUFFER_BLOCK);

	ol_prefix_size = parse_ordered_list_initial_prefix(ob, data, size);
	...
	if (doc->md.list)
		doc->md.list(ob, work, flags, ol_prefix, &doc->data);
	free(ol_prefix);
	popbuf(doc, BUFFER_BLOCK);
	return i;
}
